    ...
    OwnableUpgradeable
{
     ...
     * @dev Constructor
     */
    function initialize(address[] memory _rewardTokens) public initializer {
    ...
    function claimableRewards(
        ...
        for (uint256 i; i < rewardTokens.length; i++) {
            rewardAmounts[i] =
                claimable[rewardTokens[i]][account] +
                _earned(account, rewardTokens[i]) /
                1e50;
        }
    ...
    function _unstake(uint256 amount, address onBehalfOf) internal {
        UserData storage userInfo = userData[onBehalfOf];
        if (userInfo.tokenAmount < amount) revert InvalidAmount();
        _updateReward();
        for (uint i; i < rewardTokens.length; i++) {
            _calculateClaimable(onBehalfOf, rewardTokens[i]);
        }
    ...
    function gaugeEarned(address token) public view returns (uint earned) {
        // get for first position
        int24 tick0 = IHypervisor(stakingToken).baseLower();
        int24 tick1 = IHypervisor(stakingToken).baseUpper();

        bytes32 positionHash = IGauge(gauge).positionHash(
            stakingToken,
            0,
            tick0,
            tick1
        ...
        uint lastClaim = IGauge(gauge).lastClaimByToken(token, positionHash);
        uint currentPeriod = block.timestamp / 1 weeks;

        for (uint i = lastClaim; i <= currentPeriod; ++i) {
            earned += IGauge(gauge).periodEarned(
                i,
                token,
                stakingToken,
                0,
        ...
        // get again for limit lower/upper
        tick0 = IHypervisor(stakingToken).limitLower();
        tick1 = IHypervisor(stakingToken).limitUpper();

        positionHash = IGauge(gauge).positionHash(
            stakingToken,
            0,
            tick0,
            tick1
        ...
        // not updating lastClaim cause they are claimed at the same time
    ...
    function _calculateClaimable(
        ...
        if (userInfo.lastTimeUpdated > 0 && userInfo.tokenAmount > 0) {
            claimable[_rewardToken][_onBehalf] +=
                ((r.rewardPerToken - userInfo.rewardPerToken[_rewardToken]) *
                    userInfo.tokenAmount) /
                1e50;
        }
